...
const SUPA_URL =
  Constants.expoConfig?.extra?.EXPO_PUBLIC_SUPA_URL;
const SUPA_PUBLISH_API_KEY =
  Constants.expoConfig?.extra?.EXPO_PUBLIC_SUPA_PUBLISH_API_KEY;
...
export default function ArticleDetail() {
  ...
  const callOnDemandSummarizer = async () => {
    if (!SUPA_URL || !SUPA_PUBLISH_API_KEY) {
      console.log("Missing SUPABASE_URL or SUPABASE_PUBLISH_KEY in config");
      return;
    }
    ...
    try {
      ...
      const resp = await fetch(
        `${SUPA_URL}/functions/v1/on-demand-summarize-article`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${SUPA_PUBLISH_API_KEY}`,
          },
